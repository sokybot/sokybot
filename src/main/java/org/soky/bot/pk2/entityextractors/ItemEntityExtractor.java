...
public class ItemEntityExtractor implements IPK2EntityExtractor<Set<ItemEntity>> {
	...
	private IPk2Reader pk2Reader;

	public ItemEntityExtractor(IPk2Reader pk2Reader) {
		this.pk2Reader = pk2Reader;
	}
	...
	private String[] extractItemFileNames() {
		...
		byte[] fileBytes = this.pk2Reader.getFileBytes("itemdata.txt");
	...
	private Set<ItemEntity> extractItemDataAt(String fileName) throws IOException {
		...
		List<JMXFile> matchedFiles = this.pk2Reader.find("(?i)" + fileName );
		...
		for(JMXFile file : matchedFiles ) { 
			...
			byte[] fileBytes = this.pk2Reader.getFileBytes(file);
